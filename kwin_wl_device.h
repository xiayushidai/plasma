...
class KWinWaylandDevice : public QObject
{
    ...
    Q_PROPERTY(bool naturalScroll READ isNaturalScroll WRITE setNaturalScroll NOTIFY naturalScrollChanged) //设置naturalScroll的属性。
    //读函数：isNaturalScroll
    //写函数：setNaturalScroll
    //信号：naturalScrollChanged
...
public:
    ...
    bool naturalScrollEnabledByDefault() const {
        return m_naturalScrollEnabledByDefault.val;  //返回默认值
    }
    bool isNaturalScroll() const {
        return m_naturalScroll.val;  //返回当前的值，判断是否为natural.
    }
    void setNaturalScroll(bool set) { //设置当前的natural值。
...
Q_SIGNALS:
    ...
    void naturalScrollChanged(); //是否变化的信号。
